...
export class ThemeEditorPanel {
	...
	private handleNavigateToSetting(setting: string): void {
...
<span class="token keyword">const</span> <span class="token function-variable function">isValid</span> <span class="token operator">=</span> <span class="token punctuation">(</span><span class="token parameter">value</span><span class="token punctuation">:</span> <span class="token builtin">any</span><span class="token punctuation">)</span> <span class="token operator">=></span> <span class="token parameter">value</span> <span class="token operator">!=</span> <span class="token keyword">null</span><span class="token punctuation">;</span>
	...
	private generateWorkbenchColorsSection (currentTheme: ThemeDefinition, templateTheme: ThemeDefinition): string {
		...
		for (const [categoryName, keys] of Object.entries(categories)) {
			...
			keys.forEach((key: string) => {
				...
				if (requiresTransparency) itemClasses.push('requires-transparency');
	...
	private getTooltipForSetting(key: string): string {
	...
	private getNavigationTarget(key: string): string {
	...
	*/
	...
	private requiresTransparency(key: string): boolean {
	...
	private requiresSpecificSetting(key: string): boolean {
	...
	private getTextMateTokenExample(scope: string): string {
		...
		if (examples[scope]) return examples[scope];
		
		// Try partial matches for scopes with dots
		const parts = scope.split('.');
		for (let i = parts.length - 1; i >= 0; i--) {
			const partialScope = parts.slice(0, i + 1).join('.');
			if (examples[partialScope]) return examples[partialScope];
		}
		
		// Fallback based on scope name patterns
		if (scope.includes('string')) return '"..."';
		if (scope.includes('number')) return '123';
		if (scope.includes('comment')) return '//';
		if (scope.includes('keyword')) return 'key';
		if (scope.includes('function')) return 'fn';
		if (scope.includes('class')) return 'Cls';
		if (scope.includes('variable')) return 'var';
		if (scope.includes('type')) return 'Type';
		if (scope.includes('operator')) return '+';
		if (scope.includes('punctuation')) return '()';
		
		// Final fallback
		return 'Abc';
	}

	private getSemanticTokenExample(key: string): string {
		const examples: Record<string, string> = {
			// Language constructs
			'keyword': 'if',
			'keyword.control': 'for',
		...
		for (let i = parts.length - 1; i >= 0; i--) {
			...
			if (examples[partialKey]) return examples[partialKey];
		}

		// Pattern-based fallbacks
		if (key.includes('function')) return 'fn()';
		if (key.includes('class')) return 'Class';
		if (key.includes('variable')) return 'var';
		if (key.includes('string')) return '"..."';
		if (key.includes('number')) return '42';
		if (key.includes('keyword')) return 'key';
	 ...
	 */
	private async handleOpenSetting(settingName: string): Promise<void> {
		try {
